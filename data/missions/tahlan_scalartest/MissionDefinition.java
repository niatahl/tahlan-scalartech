package data.missions.tahlan_scalartest;
...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "ScalarTech Defense Force");
		...
		for (String testID : Global.getSettings().getAllVariantIds()) {
			if (Global.getSettings().getVariant(testID).hasHullMod("tahlan_scalarspecs") && Global.getSettings().getVariant(testID).getSource() == VariantSource.HULL && !Global.getSettings().getVariant(testID).getHullSize().equals(ShipAPI.HullSize.FIGHTER)) {
