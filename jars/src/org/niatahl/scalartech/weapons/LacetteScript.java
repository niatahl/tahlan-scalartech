...
public class LacetteScript implements EveryFrameWeaponEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (chargelevel >= 1f && !hasFiredThisCharge) {
            ...
            if (currentBarrel == 0) {
                Global.getCombatEngine().spawnExplosion(point, new Vector2f(0f, 0f), PARTICLE_COLOR, 10f, 0.1f);
                //Global.getCombatEngine().spawnExplosion(point, new Vector2f(0f, 0f), FLASH_COLOR, 15f, 0.2f);
                engine.addSmoothParticle(point, ZERO, 20f, 0.7f, 0.1f, PARTICLE_COLOR);
                engine.addSmoothParticle(point, ZERO, 30f, 0.7f, 0.5f, GLOW_COLOR);
                //engine.addHitParticle(point, ZERO, 200f, 1f, 0.05f, FLASH_COLOR);
                currentBarrel = 1;
            } else {
                Global.getCombatEngine().spawnExplosion(point2, new Vector2f(0f, 0f), PARTICLE_COLOR, 10f, 0.1f);
                //Global.getCombatEngine().spawnExplosion(point2, new Vector2f(0f, 0f), FLASH_COLOR, 15f, 0.2f);
                engine.addSmoothParticle(point2, ZERO, 20f, 0.7f, 0.1f, PARTICLE_COLOR);
                engine.addSmoothParticle(point2, ZERO, 30f, 0.7f, 0.5f, GLOW_COLOR);
